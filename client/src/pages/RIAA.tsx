...
/* ─── Reasoning Widget (image viewer) ──────────────────────────────────── */
// Add future annotated images to this array in order — the panel is scrollable.
...
function RIAAReasoningWidget() {
  ...
  return (
    <>
      ...
      {/* ── Modal — centered, scrollable, full image display ── */}
      <AnimatePresence>
        {open && (
          <motion.div
          ...
          >
            <motion.div
              className="relative bg-white rounded-2xl shadow-2xl overflow-hidden pointer-events-auto flex flex-col"
              style={{
            ...
            >
              {/* Sticky header */}
              <div
                className="shrink-0 flex items-center justify-between px-5 py-3 border-b border-[#e5e7eb]"
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Full transparency into what the system understood, how it scored, and what it did.
                  </p>
                </div>
                <button
                  onClick={() => setOpen(false)}
                  className="ml-4 p-2 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors shrink-0"
                  aria-label="Close"
                >
                  <X className="w-5 h-5" />
                </button>
              ...
              {/* Scrollable image area — one image per slide, stacked vertically */}
              ...
              {/* Footer hint */}
              <div className="shrink-0 px-5 py-2.5 border-t border-[#e5e7eb] bg-slate-50 flex items-center justify-between">
                <span className="text-xs text-slate-400">
                  {REASONING_SLIDES.length > 1
                    ? `${REASONING_SLIDES.length} annotated views — scroll to see all`
